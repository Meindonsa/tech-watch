...
import com.meindonsa.tech_watch.shared.PaginatedResponse;
...
public class ArticleService implements IArticleService{
    ...
    public ArticlesView retrieveArticles(PaginatedRequest request) {
        Source source = Functions.isNullOrEmpty(request.getFid()) ? null: sourceService.retrieveByFunctionalID(request.getFid());
        Pageable pageable = Utils.getPaging(request);
        Page<Article> articles = articleDao.search(Utils.getString(request.getSearchKey()),source, pageable);
